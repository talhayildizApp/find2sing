...
class _ChallengeModeSelectScreenState extends State<ChallengeModeSelectScreen>
                ...
                Expanded(
                  child: AnimatedDefaultTextStyle(
                    ...
                    style: TextStyle(
                      ...
                      color: hasSelection
                          ? Colors.white.withValues(alpha: 0.9)
                          : const Color(0xFF394272).withValues(alpha: 0.75),
                      height: 1.3,
                    ),
                    child: Text(
                      widget.challenge.type == ChallengeType.artist
                          ? 'Bu sanatçının şarkılarını ne kadar iyi biliyorsun?'
                          : 'Bu kategorideki şarkıları tahmin et!',
                    ),
                  ),
                ),
              ],
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            if (!isSelected && !isOtherSelected && mode == ChallengeSingleMode.timeRace) ...[
                              const SizedBox(width: 8),
                              Container(
  ...
  String _getRuleIcon(String rule) {
    ...
    if (rule.contains('Hız')) return '🚀';
    if (rule.contains('Sakin') || rule.contains('tempo')) return '😌';
    if (rule.contains('Toplam') || rule.contains('süre')) return '📊';
    if (rule.contains('Global') || rule.contains('skor')) return '🌍';
    return '📌';
  }
